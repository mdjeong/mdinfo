  ...
  const fetchArticles = useCallback(async (pageNum: number, append = false) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/articles/?skip=${skip}&limit=${ITEMS_PER_PAGE}&category=paper`,
  ...
  return (
    <main id="main-content">
      ...
      {articles.length === 0 && !loading ? (
        ...
        <section aria-label="논문 목록">
          <p className="results-info">{filteredArticles.length}개 논문</p>
          ...
          {hasMore && !searchQuery && selectedSource === 'all' && (
            <div className="load-more-container">
              ...
              <p className="load-more-info">{totalLoaded}개 논문 로드됨</p>
            </div>
          )}
        </section>
      )}
